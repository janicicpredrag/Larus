                          ...
                          vector< pair<Fact, DNFFormula> >& definitions) const
{
    ...
    else
    {
        ...
        for(size_t i=1; i < numPremises; i++) // todo: we should reorder the facts, so we get a smaller arity at the end
        {
            ...
            output.push_back(pair<CLFormula,string>(axiom, name+"AuxConjPrem"+std::to_string(count_aux++)));
        }
        premises.Clear();
 ...
 //       premises.Add(GetPremises().GetElement(numPremises-1));
    ...
    for(size_t i=0; i < numGoalDisjuncts; i++) {
        ...
        if (numConjuncts > 1) {
            ...
            for(size_t j=0; j < numConjuncts; j++) {
                ...
                output.push_back(pair<CLFormula,string>(axiom, name+"AuxConjDisj"+std::to_string(count_aux++)));
            }
        }
        else {
    ...
    if (numGoalDisjuncts > 2) {
        ...
        for(size_t i=1; i < numGoalDisjuncts-1; i++)  {
            ...
            output.push_back(pair<CLFormula,string>(axiom, name+"AuxDisj"+std::to_string(count_aux++)));
        }
        ConjunctionFormula conj1;
        conj1.Add(current);
        ConjunctionFormula conj2;
...
void CLFormula::NormalizeGoal(const string& name, const string& suffix, vector< pair<CLFormula,string> >& output) const
{
    ...
    for(size_t i=0; i < numGoalDisjuncts; i++) {
        ...
        if (numConjuncts > 1) {
            ...
            output.push_back(pair<CLFormula,string>(axiom, name+"AuxGoal"+std::to_string(count_aux++)));
        }
        else {
            disjuncts[i] = GetGoal().GetElement(i).GetElement(0);
        }
