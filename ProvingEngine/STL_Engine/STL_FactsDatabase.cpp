...
void STLFactsDatabase::AddCases(const DNFFormula& f) {
    ...
    if (f.GetSize() == 1) {
        for (vector<Fact>::const_iterator jt = f.GetDNF()->begin()->GetConjunction().begin(); jt != f.GetDNF()->begin()->GetConjunction().end(); ++jt) {
            AddFact(*jt);
        }
    }
    else
...
bool STLFactsDatabase::ApplyExcludedMiddle(DNFFormula& fout)
{
    ...
    for(set<string>::iterator it = mpT->mConstantsPermissible.begin(); it!=mpT->mConstantsPermissible.end(); it++) {
        for(set<string>::iterator jt = it; jt!=mpT->mConstantsPermissible.end(); jt++) {
            for(set<string>::iterator kt = jt; kt!=mpT->mConstantsPermissible.end(); kt++) {
                DNFFormula d;
                if (it==jt || kt==jt)
                    continue;
                string eqs("betS");
...
bool STLFactsDatabase::ApplyAxiom(const CLFormula& cl, ConjunctionFormula& fin, DNFFormula& fout, vector<pair<string,string>>& orderedInstantiation)
{
    ...
    for (set<Fact>::iterator it=mDatabase.begin(); it != mDatabase.end(); ++it)  {
        ...
        if (f.GetName() == "eq")
            f.SetName("neq");
        else if (f.GetName() == "betS")
            f.SetName("nbetS");
        else
            continue;
        for (set<Fact>::iterator jt=mDatabase.begin(); jt != mDatabase.end(); ++jt)  {
            vector<Fact> AuxFacts;
            if (MatchFact(f,*jt,instantiation, true, AuxFacts)) {
                fin.Add(*it);
                fin.Add(*jt);
                for (vector<Fact>::iterator kt = AuxFacts.begin(); kt!=AuxFacts.end(); kt++)
                    fin.Add(*kt);
                Fact b;
                b.SetName("false");
    ...
    if (FindInstantiationOfUnivVars(cl, fin, current, VarsNotInPremises, instantiation)) {
        do {
           /* cout << "no of instantiations: " << ++noOfInstantiations << endl;
             ...
             if (HoldsDisjunction(fout, conj, AuxFacts)) {
                //    cout << "Conjunction " << fout << " already holds, nothing added"  << endl;
             }
             else {
...
{
    return (mDatabase.find(f) != mDatabase.end());
}
...
bool STLFactsDatabase::FindInstantiationOfUnivVars(const CLFormula& cl, ConjunctionFormula& conj_inst, vector<std::set<Fact>::iterator>& current, map<string, set<string>::iterator>& VarsNotInPremises, map<string,string>& instantiation)
{
  ...
  //      return true;
    ...
    return NextInstantiationOfUnivVars(cl, conj_inst, current, VarsNotInPremises, instantiation);
}
...
bool STLFactsDatabase::NextInstantiationOfUnivVars(const CLFormula& cl, ConjunctionFormula& conj_inst, vector<std::set<Fact>::iterator>& current, map<string, set<string>::iterator>& VarsNotInPremises, map<string,string>& instantiation)
{
   ...
   //     return false;
    if (mpT->mConstantsPermissible.empty() && VarsNotInPremises.size()!=0)
        return false;
    if (cl.GetPremises().GetSize() == 1 && cl.GetPremises().GetElement(0).GetName() == "true" && VarsNotInPremises.size()==0)
        return false;
    ...
    do {
        ...
        else
        {
            ...
            while(!simple_check || !PremisesTrueInInstantiation(cl, conj_inst, current, instantiation));
            return true;
        }
    }
...
bool STLFactsDatabase::HoldsDisjunction(const DNFFormula& dnf, ConjunctionFormula& fin, vector<Fact>& AuxFacts)
{
    ...
    for (std::vector<pair<DNFFormula, unsigned>>::const_iterator it = mDatabaseCases.begin(); it != mDatabaseCases.end(); it++) {
...
bool STLFactsDatabase::MatchConjunction(const ConjunctionFormula& conj, ConjunctionFormula& conj_inst, size_t i, map<string,string>& instantiation, bool checkingPremises, vector<Fact>& AuxFacts)
{
    if (i==conj.GetSize())
        return true;
...
bool STLFactsDatabase::MatchFact(const Fact& f1, const Fact& f2, map<string,string>& instantiation, bool checkingPremises, vector<Fact>& AuxFacts)
{
    ...
    if (checkingPremises)  {
        for (size_t j=0; j<f1s; j++)  {
            ...
            for(std::set<string>::iterator it = mpT->mConstantsPermissible.begin(); !found && it != mpT->mConstantsPermissible.end(); it++)
                //cout << "#" << *it << "#" << endl;
                if (*it == f2.GetArg(j))
                    found = true;
...
bool STLFactsDatabase::PartialMatchFact(const Fact& f1, const Fact& f2, map<string,string>& instantiation) const
{
    size_t f1s = f1.GetArity();
    if (f1s != f2.GetArity() || f1.GetName() != f2.GetName())
        return false;
