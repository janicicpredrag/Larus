...
Variable gap1_3 : (i_ a1_ b2b3_ -> goal_).
Variable gap2_4 : (i_ b3_ a1a2_ -> goal_).
...
Variable unique_21 : (forall P L M Q : Universe, ((i_ P L /\ (i_ P M /\ (i_ Q L /\ i_ Q M))) -> (e_ P Q \/ e_ L M))).
Variable join_22 : (forall P Q : Universe, ((p_ P /\ p_ Q) -> (exists L : Universe, (dom_ L /\ (i_ P L /\ i_ Q L))))).
Variable meet_23 : (forall L M : Universe, ((l_ L /\ l_ M) -> (exists P : Universe, (dom_ P /\ (i_ P L /\ i_ P M))))).
Variable papp_24 : (forall A L B C D M E F N G O X H Y S I T : Universe, ((i_ A L /\ (i_ B L /\ (i_ C L /\ (i_ D M /\ (i_ E M /\ (i_ F M /\ (i_ B N /\ (i_ F N /\ (i_ G N /\ (i_ C O /\ (i_ E O /\ (i_ G O /\ (i_ C X /\ (i_ D X /\ (i_ H X /\ (i_ A Y /\ (i_ F Y /\ (i_ H Y /\ (i_ A S /\ (i_ E S /\ (i_ I S /\ (i_ B T /\ (i_ D T /\ i_ I T))))))))))))))))))))))) -> (exists U : Universe, (e_ N O \/ (e_ X Y \/ (e_ S T \/ (dom_ U /\ (i_ G U /\ (i_ H U /\ i_ I U))))))))).

Theorem cro_case1_25 : goal_.
Proof.
  time tac.
Qed.
