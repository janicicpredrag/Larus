...
fof(lemma_mid_plgs, axiom, (! [A,B,C,D,M] : (((~col(A,B,C) & midpoint(A,M,C) & midpoint(B,M,D))) => pG(A,B,C,D)))).
fof(lemma_parallelogram_cong, axiom, (! [A,B,C,D] : ((pG(A,B,C,D) => (cong(A,B,C,D) & cong(B,C,D,A)))))).
fof(lemma_par_cong_os_parallelogram, axiom, (! [A,B,C,D]: ((par(A,B,C,D) & cong(A,B,C,D) & os(A,B,C,D)) => pG(A,B,C,D)))).
fof(th_triangle_mid_par_strict,conjecture,(! [A,B,C,P,Q,R,X] : (( ( (~ col(A,B,C))  & (A != C) & (C!=A) & (~col(A,P,C)) & midpoint(B,P,C) & midpoint(A,Q,C) & midpoint(A,R,B) & midpoint(P,Q,X) & os(A,X,P,B)) => ( pG(A,P,C,X) & cong(A,X,B,P) & par(A,X,B,P) & pG(A,X,P,B) & tP(A,B,Q,P)))))).
