fof(lemma_notperp,axiom, (! [A,B,C,P] : (? [X] : ((between(A,C,B) & ~ (collinear(A,B,P))) => ((~ (collinear(A,B,X)) & same_side(X,P,A,B) & ~ (right_angle(A,C,X)))))))).
...
fof(propsame_sideition_11B,conjecture,(! [A,B,C,P] : (? [X] : ((between(A,C,B) & ~ (collinear(A,B,P))) => ((right_angle(A,C,X) & opposite_sides(X,A,B,P))))))).
