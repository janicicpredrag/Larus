...
fof(propsame_sideition_30A,conjecture,(! [A,B,C,D,E,F,G,H,K] : ((parallel(A,B,E,F) & parallel(C,D,E,F) & between(G,H,K) & between(A,G,B) & between(E,H,F) & between(C,K,D) & opposite_sides(A,G,H,F) & opposite_sides(F,H,K,C)) => ((parallel(A,B,C,D)))))).
