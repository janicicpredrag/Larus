...
Variable tOr_27_ : Universe -> Universe -> Prop.
...
Variable initial_model_1 : (dom_ Y /\ (dom_ X /\ (tOr_25_ Y X /\ tOr_27_ Y X))).
...
Variable ax00_4 : (forall Y X : Universe, ((tOr_27_ Y X /\ (fQ_ X Y /\ fQ_ Y X)) -> goal_)).
...
Variable ax09_13 : (forall Y Z X : Universe, ((dom_ Y /\ fP_ Z X) -> fAnd_6_ X Z Y)).
Variable ax10_14 : (forall X Z Y : Universe, ((dom_ X /\ fP_ Z Y) -> fAnd_6_ X Z Y)).
Variable ax11_15 : (forall X Y : Universe, (tExists_21_ X Y -> (exists Z : Universe, (dom_ Z /\ (tOr_15_ X Z Y /\ tOr_17_ X Z Y))))).

Theorem problem4340_16 : goal_.
Proof.
  time tac.
Qed.
