...
Variable proposition_29B_15 : (forall A D G H : Universe, ((par_ A G H D /\ tS_ A G H D) -> congA_ A G H G H D)).
Variable lemma_ABCequalsCBA_16 : (forall A B C : Universe, (~(col_ A B C) -> congA_ A B C C B A)).
Variable lemma_equalanglestransitive_17 : (forall A B C D E F P Q R : Universe, ((congA_ A B C D E F /\ congA_ D E F P Q R) -> congA_ A B C P Q R)).
...
Variable proposition_27B_22 : (forall A D E F : Universe, ((congA_ A E F E F D /\ tS_ A E F D) -> par_ A E F D)).

Theorem proposition_33_23 : (forall A B C D M : Universe, ((par_ A B C D /\ (cong_ A B C D /\ (betS_ A M D /\ betS_ B M C))) -> (par_ A C B D /\ cong_ A C B D))).
Proof.
  time tac.
Qed.
