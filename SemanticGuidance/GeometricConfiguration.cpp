        ...
        if (!overconstrained) {
            do {
                ...
                for (auto it = inputConfiguration.begin(); it != inputConfiguration.end(); it++)  {
                    ...
                    if (FactToConstraints(inputConfiguration, *it, degreesOfFreedom)) {
                        *it = null_fact;
                        cout << endl << "Update:   ";
                        printCurrentStatus(inputConfiguration, degreesOfFreedom);
...
bool GeometryConfiguration::FactToConstraints(vector<Fact>& inputConfiguration, const Fact& f, map<string, int>& degreesOfFreedom) {
    string A[6];
