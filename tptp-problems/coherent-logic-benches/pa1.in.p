...
fof(mnotplus,axiom, ![ X, Y, Z] : ((my_plus(X,Y,Z) & mnotplus(X,Y,Z)) =>  goal )).
fof(plusnull,axiom, ![ X] : (nat(X) => my_plus(X,num_0,X) )).
fof(plussucc,axiom, ![ X, Y, Z, Y1, Z1] : ((my_plus(X,Y,Z) & s(Y,Y1) & s(Z,Z1)) => my_plus(X,Y1,Z1) )).
fof(induction_instance,axiom, ![ X] : ((nat(X) & my_plus(num_0,num_0,num_0)) => ? [Y] :  (my_plus(num_0,X,X)| ((dom(X) & dom(Y) & my_plus(num_0,X,X) & s(X,Y) & mnotplus(num_0,Y,Y)))) )).
