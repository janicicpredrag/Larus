fof(lemma_notperp,axiom, (! [A,B,C,P] : (? [X] : ((betS(A,C,B) & ~ (col(A,B,P))) => ((~ (col(A,B,X)) & oS(X,P,A,B) & ~ (per(A,C,X)))))))).
...
fof(proposition_11B,conjecture,(! [A,B,C,P] : (? [X] : ((betS(A,C,B) & ~ (col(A,B,P))) => ((per(A,C,X) & tS(X,A,B,P))))))).
